...
public class kNN_GA {
    ...
    public boolean[][] crossover(boolean [] p1,boolean [] p2 ){
        ...
        Random r = new Random();
        double randomProbability=r.nextDouble(1.0);

        if(randomProbability<CROSSOVER_PROBABILITY){
            boolean multipointCrossover= r.nextBoolean();
            int crossOverPoint1 = r.nextInt(FEATURE_SIZE);
            ...
            if(multipointCrossover){
                ...
                int crossOverPoint2 = r.nextInt(FEATURE_SIZE);
    ...
    public boolean[] mutation(boolean [] child){
        ...
        if(randomProbability<MUTATION_PROBABILITY){
            int mutationPoint = r.nextInt(FEATURE_SIZE);
            mutatedChild[mutationPoint]=!(mutatedChild[mutationPoint]);
        }
        return mutatedChild;
    }
    public boolean[][] evolve(boolean[][] sol, double fitness[]){
    ...
    //randomly select 2 from temp_newPopulation and send it for crossover then send each of them for mutation
    //perform mutation
    //perform crossover
     return newPopulation;
    }
    private boolean[] tournamentSelection(boolean[][] sol,double[] fitness){
        Random r = new Random();
        int maxAt = 0;
        ...
        for(int i=0;i<TOURNAMENT_SELECTION_SIZE;i++){
            int selectionIndex=r.nextInt(99);
